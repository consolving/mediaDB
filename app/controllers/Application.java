...
public class Application extends Controller {
	...
	protected final static Integer PER_PAGE = ConfigFactory.load().getInt("results.per_page");
	...
	public static Result index() {
		...
		return ok(
				index.render(mediaFilesCount, thumbnailsCount, tagCount, propertyCount, PER_PAGE, page - 1, prev, next));
	}
	...
	protected static Boolean getQuereparameterAsBoolean(String key, Boolean defaultValue) {
		Boolean value = request().getQueryString(key) != null ? Boolean.parseBoolean(request().getQueryString(key))
				: null;
		return value != null ? value : defaultValue;
	}

	protected static String getQuereparameterAsString(String key, String defaultValue) {
		String value = request().getQueryString(key) != null ? request().getQueryString(key) : null;
		return value != null ? value : defaultValue;
	}
